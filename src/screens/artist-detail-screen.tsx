...
import { getArtistById, getArtistSongs, Song, getImageUrl } from '../services/api';
  ...
  useEffect(() => {
    const fetchArtistData = async () => {
      ...
      try {
        ...
        setSongs(artistSongs.slice(0, 10)); // Show top 10 songs
      } catch (error) {
    ...
    <SafeAreaView className={`flex-1 ${colorScheme === 'dark' ? 'bg-black' : 'bg-white'}`}>
      <ScrollView showsVerticalScrollIndicator={false}>
        ...
        <Text className="text-center text-gray-500 mt-2 text-base">
          {artistDetails?.topAlbums?.length || artist.albumCount} Album  |  {songs.length} Songs  |  {Math.floor(totalMins / 60)}:{(totalMins % 60).toString().padStart(2, '0')}:{(totalDuration % 60).toString().padStart(2, '0')} mins
        </Text>
              ...
              {songs.map((song) => (
                <View key={song.id} className="flex-row items-center mb-4">
                  ...
                  <View className="flex-1 ml-3">
                    ...
                    <Text className="text-gray-500 text-sm mt-1">{artist.name}</Text>
                  </View>
                  <TouchableOpacity 
